...
//웹 통신을 위한 코드 (61 line ~ 84)(수정)
let tdSocket = null;

function connectToBridge() {
  tdSocket = new WebSocket("ws://localhost:8080");

  tdSocket.onopen = () => {
    console.log("Connected to TD bridge");
  };

  tdSocket.onclose = () => {
    console.log("Disconnected from TD bridge");
  };
...
function sendToTouchDesigner(data) {
  if (tdSocket && tdSocket.readyState === WebSocket.OPEN) {
    tdSocket.send(JSON.stringify(data));
  }
}
...
const TASKS = [
  ...
  },
  {
    id: "side_step",
    name: "Side Step",
    instruction: "Step left, return, then step right, return.",
    guideCaption: "Move one foot to the side, come back, then repeat on the other side.",
    demoType: "side_step"
  }
];
...
function scoreAirplaneArms(landmarks, baseline) {
  ...
  if (
    !ls || !rs || !le || !re || !lw || !rw ||
    !baseline?.leftWrist || !baseline?.rightWrist
  ) {
  ...
  const spreadGate = clamp01((Math.min(leftOut, rightOut) - 0.10) / 0.16);
...
function updateStepSequence(record, dir) {
  if (dir === "center") return;
  const last = record.stepSequence[record.stepSequence.length - 1];
  if (last !== dir) {
    record.stepSequence.push(dir);
    if (record.stepSequence.length > 8) record.stepSequence.shift();
  }
}

function stepSequenceMatches(record) {
  return record.stepSequence.join(",").includes("left,right");
}

function scoreSideStep(landmarks, baseline, record) {
  const lh = landmarks[23];
  const rh = landmarks[24];
  ...
  if (
    !lh || !rh || !lk || !rk || !la || !ra || !ls || !rs ||
    !baseline?.leftHip || !baseline?.rightHip ||
    !baseline?.leftKnee || !baseline?.rightKnee ||
    !baseline?.leftAnkle || !baseline?.rightAnkle ||
    !baseline?.leftShoulder || !baseline?.rightShoulder
  ) {
    return { score: 0, poseScore: 0, holdReady: false, motionGate: 0 };
  }
  ...
  const bodyShift = (hipMid.x - baseHipMid.x) / Math.max(scale, 0.08);
  const shoulderShift = (shoulderMid.x - baseShoulderMid.x) / Math.max(scale, 0.08);

  const leftFootOut = (baseline.leftAnkle.x - la.x) / Math.max(scale, 0.08);
  const rightFootOut = (ra.x - baseline.rightAnkle.x) / Math.max(scale, 0.08);

  const leftKneeOut = (baseline.leftKnee.x - lk.x) / Math.max(scale, 0.08);
  const rightKneeOut = (rk.x - baseline.rightKnee.x) / Math.max(scale, 0.08);

  const leftFootScore = clamp01((leftFootOut - 0.03) / 0.18);
  const rightFootScore = clamp01((rightFootOut - 0.03) / 0.18);
  const leftLegSupport = clamp01((leftKneeOut - 0.01) / 0.10);
  const rightLegSupport = clamp01((rightKneeOut - 0.01) / 0.10);

  const leftShiftFollow = clamp01((-(bodyShift + shoulderShift * 0.4) - 0.01) / 0.08);
  const rightShiftFollow = clamp01(((bodyShift + shoulderShift * 0.4) - 0.01) / 0.08);

  const leftStepScore =
    leftFootScore * 0.58 + leftLegSupport * 0.20 + leftShiftFollow * 0.22;
  const rightStepScore =
    rightFootScore * 0.58 + rightLegSupport * 0.20 + rightShiftFollow * 0.22;

  record.leftStepPeak = Math.max(record.leftStepPeak, leftStepScore);
  record.rightStepPeak = Math.max(record.rightStepPeak, rightStepScore);

  let dir = "center";
  if (leftStepScore > rightStepScore && leftStepScore > 0.44) dir = "left";
  if (rightStepScore > leftStepScore && rightStepScore > 0.44) dir = "right";
  updateStepSequence(record, dir);

  const patternScore = stepSequenceMatches(record)
    ? 1
    : clamp01((record.leftStepPeak + record.rightStepPeak) / 1.35);

  const bilateralScore = Math.min(
    1,
    Math.min(record.leftStepPeak, record.rightStepPeak) / 0.68
  );

  const amplitudeScore = clamp01(
    (Math.max(record.leftStepPeak, record.rightStepPeak) - 0.40) / 0.35
  );
  ...
  const poseScore =
    ...
    bilateralScore * 0.28 +
    amplitudeScore * 0.20 +
    Math.max(leftShiftFollow, rightShiftFollow) * 0.12;

  const motionGate = clamp01(
    (Math.max(leftStepScore, rightStepScore, Math.abs(bodyShift)) - 0.16) / 0.32
  );

  const score = 100 * motionGate * poseScore;
  const holdReady = stepSequenceMatches(record) && bilateralScore > 0.60 && poseScore > 0.68;
...
function scoreCurrentTask(landmarks, baseline, record) {
  ...
  if (task.id === "side_step") {
    return scoreSideStep(landmarks, baseline, record);
  }
...
function finalizeCurrentTask() {
  ...
  record.finalTaskScore =
    record.bestPoseScore * 100 * 0.50 +
    stability * 100 * 0.30 +
    reactionScore * 100 * 0.20;
...
function drawSilhouette(base) {
  ...
  drawThickLine(p.shoulderLine[0], p.shoulderLine[1], 12);
...
function createGuidePose(type, e, timeMs = 0) {
  ...
  if (type === "side_step") {
    ...
    if (cycle < 0.25) {
      leftStep = easeInOut(cycle / 0.25);
    } else if (cycle < 0.5) {
      leftStep = 1 - easeInOut((cycle - 0.25) / 0.25);
    } else if (cycle < 0.75) {
      rightStep = easeInOut((cycle - 0.5) / 0.25);
    } else {
      rightStep = 1 - easeInOut((cycle - 0.75) / 0.25);
    }

    // left step
    base.leftAnkle.x = lerp(0.47, 0.29, leftStep);
    base.leftKnee.x = lerp(0.46, 0.38, leftStep);
    base.leftHip.x = lerp(0.45, 0.42, leftStep);

    // right step
    base.rightAnkle.x = lerp(0.53, 0.71, rightStep);
    base.rightKnee.x = lerp(0.54, 0.62, rightStep);
    base.rightHip.x = lerp(0.55, 0.58, rightStep);

    // body follow
    const bodyShift = -0.03 * leftStep + 0.03 * rightStep;
    base.head.x += bodyShift;
    base.neck.x += bodyShift;
    base.leftShoulder.x += bodyShift;
    base.rightShoulder.x += bodyShift;

    // natural arm swing
    base.leftElbow.x += 0.03 * rightStep - 0.02 * leftStep;
    base.leftWrist.x += 0.05 * rightStep - 0.03 * leftStep;
    base.rightElbow.x += 0.02 * leftStep - 0.03 * rightStep;
    base.rightWrist.x += 0.03 * leftStep - 0.05 * rightStep;

    // small bounce
    const bounce = Math.sin(cycle * Math.PI * 4) * 0.008;
    base.leftHip.y += Math.abs(bounce);
    base.rightHip.y += Math.abs(bounce);
    base.leftKnee.y += Math.abs(bounce) * 0.5;
    base.rightKnee.y += Math.abs(bounce) * 0.5;
  }
