...
function getIdleLiveScore(landmarks, baseline) {
  ...
  let bodyVisibleScore = clamp01((shoulderWidth + torsoLength) / 0.55);

  let motionScore = 0.5;
  if (baseline?.leftWrist && baseline?.rightWrist) {
    ...
    motionScore = clamp01((Math.max(leftMove, rightMove) + 0.08) / 0.28);
  }

  const liveScore = (bodyVisibleScore * 0.7 + motionScore * 0.3) * 100;
  return liveScore;
}
...
function loop() {
  ...
  // 不管当前是什么阶段，都先显示一个实时分数
  if (state !== "performing") {
    const idleScore = getIdleLiveScore(landmarks, baselinePose);
    scoreValueEl.textContent = idleScore.toFixed(1);
  }
